...
object KotlinWorkshop3 {
    ...
    private fun countHigherGuesses(
        ...
        var counter = 0
        guesses.filter { it > randomNumber }.forEach {
            println("$counter) $it is higher than $randomNumber")
            counter++
        }
        printer(counter)
        return counter
    ...
    val countLowerGuesses: (IntArray, Int) -> Unit = { guesses, randomNumber ->
        var counter = 0
        guesses.filter { it in 0 until randomNumber }.forEach {
            println("$counter) $it is lower than $randomNumber")
            counter++
        }
